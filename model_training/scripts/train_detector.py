...
def train_detection_model():
    ...
    print(f"Training YOLOv11 nano on dataset: {dataset_yaml}")
    ...
    try:
        ...
        # Find the best model
        best_model_path = (
            output_dir / "building_number_detector" / "weights" / "best.pt"
        )
        last_model_path = (
            output_dir / "building_number_detector" / "weights" / "last.pt"
        )
...
def main():
    ...
    try:
        ...
        if validate_training_results():
            ...
            print(
                f"📋 Review training metrics and proceed to Step 2.4 (Export Detection Model)"
            )
        else:
